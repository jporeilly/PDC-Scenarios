...
*Copper State Credit Union scenario · PDC 11.0.0 · Data Profiling · Trust Score · Business Rules*
...
In Workshop 2 the `members` table had a blank PK/FK column and a Trust
...
you go one step beyond the profiler: **Business Rules** turn CSCU's
...
> **The business problem.** A CSCU analyst is about to build the
> branch-performance report on `transactions`. Is the data complete? Are
> there nulls in the amount columns? Which column is the key, and does it
> join cleanly to `accounts`? Without profiling, these are guesses.
> Profiling turns them into measured facts — row counts, null counts,
> cardinality, and discovered keys — so the analyst builds on evidence, not
> hope. And two of CSCU's obligations are already broken in the data,
> invisibly: three opted-out members still carry live marketing emails, and
> six stored CVV codes violate PCI DSS outright. Part E makes both failures
> *measurable* — the prerequisite for fixing them and proving the fix.
...
- What the Profiling options do — sampling (Sample Type, Skip Recent) and
  the parallelism threads — and why CSCU keeps the defaults.
- How CSCU's documents are profiled by Data Discovery, which extracts text,
...
- Workshops 1–3 complete — sources connected, metadata read, and the
  glossary linked.
- Two logins from the CSCU cast: `elena.ramirez` (Data Steward) runs the
  profiling and Trust Score parts, and `dana.ortiz` (**Data Developer**)
...
- The `members`, `transactions`, and `accounts` tables in `cscu_core` — the
  worked examples (you then profile all eleven tables).
- The compliance PDFs in the `cscu-documents` store — the SAR filing
  summary, the NCUA exam response, the BSA/AML independent test, and the
  PCI DSS attestation — profiled in Part D via Data Discovery.
- `assets/Copper-State-Credit-Union-Business-Rules.sql` — six ready
  conditions in PDC's three-column shape for Part E.
- The Workshop 3 glossary terms already linked to `members` (so the
...
CSCU has two kinds of assets, and each is scanned its own way. Database
tables — like `members` — are profiled with **Data Profiling**, the
structured-data scan you run in this Part. The compliance PDFs in the
`cscu-documents` store are scanned with **Data Discovery**, its
unstructured counterpart, in Part D.

1. In the Data Canvas, open `CopperState_Core_Banking` and select the
   `members` table.
2. Open **Actions**, then **Process**, then click **Start** on the Data
   ...
   `[SCREENSHOT: members — Data Profiling card]`
3. Keep the default settings. Profiling samples up to 500,000 rows, which
   suits CSCU's tables, and Extract Samples is on by default so you get
   preview values.
   `[SCREENSHOT: Data Profiling — default settings]`
4. Watch the job on the Workers page until it completes, then repeat for
   the other ten tables — `accounts`, `cards`, `transactions`, `loans`,
   `ach_payments`, `kyc_reviews`, `suspicious_activity`, `gl_entries`,
   `branches`, and `employees`. (You can select several tables at once and
...
The Profiling page exposes sampling and performance controls. CSCU's tables
...
For CSCU: leave every option at its default. The tables are far smaller
...
1. On the `members` Summary tab, the Statistics pane now shows a Row Count
   alongside the Column count, and the Last Successful Scanned date is set.
   `[SCREENSHOT: members Summary — Statistics pane with Row Count]`
2. Click a column (for example, `email`) to see column-level statistics —
   ...
   Patterns pane with RegEx suggestions. Try `ssn` too: its pattern comes
   back as a crisp NNN-NN-NNNN, which Workshop 5's identification engine
   will put to work.
   `[SCREENSHOT: email column — statistics and Data Patterns]`
3. Open the **Details** tab and the Sample Data pane (Raw and Aggregated)
   to preview real values and how often each occurs. On `transactions`,
   check `txn_amt` min/max (negatives are debits) and `txn_type_cd`'s eight
   distinct values — that enum becomes a dictionary in the Technical Track.
...
works across tables. CSCU's schema already declares every key —
`members.mbr_id` is a primary key, and `accounts`, `loans`, `kyc_reviews`
and `suspicious_activity` declare foreign keys to it — so the database is
...
then Data Profiling on every related table, not just `members`:
...
-- branches, employees, members, accounts, cards,
-- transactions, loans, ach_payments, kyc_reviews,
-- suspicious_activity, gl_entries.
-- Watch every job finish on the Workers page, then
-- reopen the Details tab on members.
```
...
1. Confirm the glossary term you linked in Workshop 3 is still attached to
   `members` — the table-level **Member Record** term. A linked term is the
   first of the four Trust Score inputs.
   `[SCREENSHOT: members table — Glossary tab]`
...
7. When the job completes, read the Key Metrics panel. With a glossary
   ...
   `[SCREENSHOT: members — Trust Score in Key Metrics]`
...
1. Confirm a glossary term is attached to the file — for example the
   Suspicious Activity Report term on
   `compliance/sar_filing_summary_2026Q2.pdf`. A linked term is the first
   of the four inputs.
2. In the file's Key Metrics panel: apply a rating, set Data Lineage to
   Verified, and set Sensitivity (Medium is reasonable for a filing
   summary; the Business Term is the check that matters).
3. Run the calculation: **Actions → Process → Start Calculate Trust
   Score**, and watch the Workers page.
   `[SCREENSHOT: Key Metrics — sar_filing_summary_2026Q2.pdf]`

### Part D — Profile the documents (Data Discovery)

The `cscu-documents` store holds CSCU's compliance PDFs, loan applications
and correspondence, not table rows — so they are scanned by **Data
Discovery**, the unstructured-data counterpart to Data Profiling. The steps
mirror Part A:

1. In the Data Canvas, open the `CopperState_Documents` store and select
...
3. The Configure Process page opens with three tabs — Data Discovery,
   ...
   CSCU, and click **Start Discovering**.
4. Watch the job on the Workers page until it completes, then repeat for
   the other folders — `loan-applications`, `statements`, `payments`,
   `correspondence` and `rates`.

Where Data Profiling computes column statistics, Data Discovery scans file
contents. For CSCU's PDFs it:
...
example, *Suspicious Activity Report* or *PCI attestation*) on the Document
Processing tab.
...
first. Discovering the `cscu-documents` PDFs here is what lets Workshop 5
flag PII and compliance content inside them.

### Part E — Business Rules: score the obligations

Profiling measured the data as it is; business rules judge it against what
it *must* be. Switch to `dana.ortiz` — in PDC v11 the **Data Developer**
...
conditions in exactly that shape, each tied to a CSCU obligation:

| Rule | Dimension | What it protects |
| --- | --- | --- |
| CSCU-Marketing-OptOut-Compliance | Conformity | **Flagship:** opted-out members must not be contactable on marketing extracts (privacy opt-out) |
| CSCU-PCI-No-Stored-CVV | Validity | PCI DSS 3.2 — no stored CVV after authorization; fails by design until the purge runs |
| CSCU-SSN-Format-Validity | Conformity | SSN must match NNN-NN-NNNN |
| CSCU-Available-Not-Above-Ledger | Consistency | Available balance must not exceed the ledger balance |
| CSCU-APR-Within-Program-Limits | Validity | 0 < APR ≤ 36% (Reg Z sanity check) |
| CSCU-ACH-Routing-Number-Format | Conformity | Nine-digit ABA routing numbers |
...
opted-out members) and **3 non-compliant** (all still carry live emails) —
exactly the gap a marketing extract must suppress.
`CSCU-PCI-No-Stored-CVV` reports **6/6 non-compliant**: the planted PCI
violation from Workshop 2, now measured. The other four rules pass —
which is just as informative, because they will keep passing (or start
failing) on their schedule, without anyone remembering to check.
`[SCREENSHOT: Business Rules dashboard — pass/fail statuses]`

## Verify your work

- [ ] All eleven `cscu_core` tables show a Row Count and a profiled date.
- [ ] Column-level statistics (Null Count, Cardinality, Uniqueness) and
      Sample Data are visible on `members` and `transactions`.
- [ ] Once every related table in the schema is profiled, the PK/FK column
      shows `mbr_id` as the primary key on `members`; the CSCU schema
      already declares the keys, so the remaining step is profiling the
      full related set.
- [ ] After Calculate Trust Score, the `members` Trust Score is no longer
      0 / Untrusted.
- [ ] The `cscu-documents` folders are discovered and their text is
      searchable.
- [ ] Six business rules are configured and evaluated; the flagship and
      PCI rules show the expected failures — and you can say why.
...
| Cannot create a business rule | In PDC v11, Business Rules are authored by the **Data Developer** role — `dana.ortiz` in the CSCU cast. The Admin role can view but not create them; a Data Steward cannot author them either. |
...
Workshop 4, the `members` Trust Score was a placeholder; now it rests on
...
binder. Discuss: which of CSCU's tables would you profile first, and what
...
and quality of CSCU's data. Workshop 5 puts that to work for compliance:
Data Identification scans the profiled tables with dictionaries and
patterns to flag PII and confidential data automatically, and you set the
Sensitivity that protects CSCU's members — including hunting down the
stored CVV codes the failing PCI rule just exposed.
