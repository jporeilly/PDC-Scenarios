...
*Copper State Credit Union scenario · PDC 11.0.0 · lab at `https://pentaho.io` (VM `192.168.1.200`)*
...
term, the Copper State Credit Union (CSCU) team needs accounts — and each
...
CSCU is a regulated credit union, and in a regulated institution *who can
change what* is as important as the data itself. A business steward who
curates the glossary should not be able to rewrite a data source; an analyst
who reads metadata should not be able to administer the system. PDC expresses
these boundaries through role-based access control, and this workshop maps
each CSCU person to the role that matches their responsibility.
...
> for what — and a single mistake can change data an examiner expects to be
> controlled. When the NCUA asks *"who decided `cvv_cd` is HIGH sensitivity,
> and when?"*, the answer must be a named steward with the right role.
> Provisioning distinct, least-privilege accounts up front makes ownership
> visible and auditable, and lets the rest of the course demonstrate real
> separation of duties rather than pretend at it.
...
- How to add each CSCU user in Data Catalog and assign the least-privilege
...
on a base role that narrow or widen access to specific assets (a Lending
community that sees only lending glossaries, for example). This workshop uses
the default roles; communities become useful once CSCU scopes access by
department.
...
- The CSCU roster below (`assets/users.csv`). All CSCU emails follow the
  pattern `name@copperstatecu.org`.
...
| Admin password | copperstate | Lab credential — set non-temporary; change in production |

## The CSCU roster — all seven PDC roles covered

The CSCU team maps one persona to each role. Business Steward is held by the
four business-domain owners; there is deliberately only **one** Data Steward.

| User | PDC role(s) | Why |
| --- | --- | --- |
| catalog.admin | ALL SEVEN ROLES | Course superuser / trainer. Password reset in Keycloak to `copperstate` |
| elena.ramirez | Data Steward · Business Steward | The single Data Steward: connects sources, profiles, runs identification; stewards Member, Accounts & Deposits, Transactions, Branch Operations |
| marcus.webb | Business Steward | Stewards Lending and Finance & Ledger terms (Loan Number, APR, GL Account) |
| nadia.flores | Business Steward | BSA/AML officer; stewards Compliance & Risk (all CDE) and Records & Documents |
| tom.callahan | Business Steward | Stewards Cards & Payments terms (Card Number, Routing Number) |
| omar.haddad | Data Storage Administrator | Storage custodian: creates/ingests data sources, monitors utilisation; fills the app's custodian slot |
| dana.ortiz | Data Developer | Authors the Workshop 4 business rules and domain logic (this role owns Business Rules — Admin cannot create them) |
...
> `data_sources/CSCU/domain_pack/credit_union.people.json`); the Glossary
...
3. Go to **Users**, search for `catalog.admin`, and open the account. If it
   ...
   `catalog.admin` and email `catalog.admin@copperstatecu.org`, and save.
4. Open the **Credentials** tab, click **Reset password**, enter
   `copperstate` in both fields, and — importantly — turn **Temporary off**
...
### Part B — Add the CSCU users (Data Catalog)

Sign in to PDC as `catalog.admin` / `copperstate`. Confirm the Administration
area is visible — proof the Admin role took effect — then add each user from
the roster.
...
3. Enter the user's details — username and email from the roster (for
   example `nadia.flores` / `nadia.flores@copperstatecu.org`).
4. Click **Add Roles**, tick the role or roles listed for that user in the
   ...
   `[SCREENSHOT: Create User — nadia.flores with Business Steward]`
5. Click **Done** to create the user. Repeat for all eight CSCU users. If you
...
2. On the **Credentials** tab, **Reset password** to the shared lab password
   `copperstate`, with Temporary turned **off**. Save.
...
> any steward role. `nadia.flores` can shape the glossary but cannot touch a
> data source; `jordan.blake` can read the catalog but cannot administer
> users; `dana.ortiz` can author business rules that even the Admin role
> cannot. That separation is exactly what an examiner expects to see, and it
...
- [ ] `catalog.admin` signs in to PDC with the password `copperstate` and
  sees the Administration area.
- [ ] All eight CSCU users exist under Users & Communities, each with the
  role(s) from the roster — covering all seven PDC roles.
- [ ] A Business Steward (for example `nadia.flores`) can open Business
  Glossary but not create a data source.
- [ ] A Data Storage Administrator (`omar.haddad`) can add a data source,
...
identity in Keycloak from authorization in PDC, and by assigning each CSCU
...
properties an examiner looks for. As the cohort moves into Workshop 1 and
beyond, keep asking the governance question this workshop sets up — not just
*"can I do this?"* but *"should this role be able to?"*

## What's next

With the cast in place, Workshop 1 connects the CSCU data estate — the
`cscu_core` core banking database and the `cscu-documents` object store —
performed by `omar.haddad`, whose Data Storage Administrator role exists for
exactly that job.
