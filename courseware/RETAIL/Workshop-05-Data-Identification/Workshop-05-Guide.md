# Workshop 5 — Data Identification (CTO)

*Canyon Trail Outfitters scenario · PDC 11.0.0 · Dictionaries & Patterns · Builds on Workshop 4 — the tables must be profiled first*
...
complete conceptual picture of how that engine works — and at CTO it has a
very concrete job: finding cardholder data wherever it hides.
...
- What the engine *finds* (`card_no` — the smoking gun) and what it
  *misses* (`loyalty_no` — the custom-method case), and why both teach the
  same lesson.
...
**Best for:** state codes, product categories, tender types, status
...
> shape, so a dictionary — not a pattern — is the correct method. (CTO's
> own enums — `category_cd`, `pay_method_cd`, `ret_reason_cd` — are exactly
> this kind of data, which is why Part B uploads CTO dictionaries for
> them.)
...
> **Worked example — the loyalty number.** The value `CTO-200101` reduces
> to the pattern `AAA-nnnnnn` — three letters, a kept dash, six digits.
> From the top ~20 patterns it finds, PDC also recommends a tunable RegEx
> (here `^CTO-\d{6}$`), which is reused later for data-quality checks that
> flag outliers.

**Best for:** email, phone, card numbers, SKUs, order numbers, IP
...
`customers.email` may not land as HIGH the way `card_no` does — the
...
your own — for CTO, the Service Cities dictionary and the Loyalty Number
pattern you'll meet below. Apply, run, then review the results in the Data
Canvas and visualize them in the Galaxy View.

## Walkthrough — identify the customers and payments tables

Run Data Identification on CTO's profiled tables (`cto_retail`), selecting
...
Workshop 3 business term. Work as `sofia.marin` — in PDC v11, Data
Identification Methods belong to the **Data Steward** and **Data Storage
Administrator** roles (Tessa's Data Developer role, which authored the
business rules, cannot author identification methods — the gates point in
opposite directions).

### Part A — Run Data Identification

1. In the Data Canvas, select the profiled `customers` table, then open
   **Actions → Process**.
...
3. Click **Select Methods** and choose the dictionaries and patterns that
   cover names, email, phone, addresses and card numbers.
   `[SCREENSHOT: Data Identification — Select Methods (the policy)]`
4. Click **Start**, and watch the job on the Workers page until it
   completes. Repeat for `payments`, `orders`, `employees` and
   `suppliers`.

### Part B — Review what was identified

1. Open `customers`. On the Details tab, the matched columns now carry Tags
   and linked Business Terms: `email` and `phone` → contact PII, the name
   columns → person-name matches. Then open `payments`: `card_no` is caught
   by the card-number pattern — the 4111… PANs match the issuer prefix and
   Luhn check, and the column lands HIGH.
   `[SCREENSHOT: identification results on payments]`
2. **The smoking gun.** That HIGH tag on `card_no` is not routine: full
   PANs should not be in this table at all. Pair the identification hit
   with Workshop 4's failing `CTO-PCI-No-Full-PAN` rule and the SAQ
   attestation PDF in `compliance/` (Requirement 3.2, NON-COMPLIANT):
   identification, quality and documents all point at the same defect.
   That triangulation is the lesson.
3. **The miss.** `customers.loyalty_no` stays untagged — no built-in method
   knows CTO's loyalty format. Built-in methods are a fast, generic
   starting point, and their limits are exactly why every organisation ends
   up building custom dictionaries and patterns. You fix this one below.

**Dictionary vs pattern, in practice:** status codes, tender types and the
opt-out flag are caught by dictionaries (their meaning is in the values);
email, phone and card numbers are caught by patterns (their meaning is in
the shape); an address takes both. The method assignments above are
illustrative — confirm them against your lab's identification run.

### Part C — Visualize in the Galaxy View

1. With `customers` selected, open **Actions → View Galaxy**.
2. Click **Filters** and turn on *Show only Tagged Items* (or items with
   ...
   classified.
   `[SCREENSHOT: Galaxy View — tagged customers columns]`
...
### Walkthrough — PII in the CTO correspondence

The `cto-documents/correspondence` folder holds emails and letters about
customers. The goal is the same as for the table: flag every document that
...
| --- | --- | --- |
| email_refund_dispute_CTO-200109 | Email | name · email · address · loyalty number · order ref |
| email_price_match_request | Email | name · email · address · loyalty number · order ref · SKU |
| letter_return_denied_SO-30000010 | Letter | name · address · loyalty number · order ref · card last-4 |
| letter_optout_confirmation_CTO-200107 | Letter | name · address · loyalty number · consent status |
| letter_backorder_apology_SO-30000002 | Letter | name · address · loyalty number · order ref · SKU |

> **A pattern reaches into the letters too.** The reference codes in the
> correspondence — `CTO-200109`, `SO-30000010` — follow fixed shapes
> (`AAA-nnnnnn`, `AA-nnnnnnnn`). A custom data pattern for those shapes
> flags CTO loyalty and order references wherever they appear in a
...
## Choosing methods for CTO

Most of CTO's data is covered by methods that ship with the catalog.
Start by selecting the built-in dictionaries and patterns that fit, then
add custom CTO methods for what the built-ins don't know.

### Recommended built-in methods — structured (customers, payments)

| CTO data | Built-in method | Type |
| --- | --- | --- |
...
| opted_out_marketing | low-cardinality flag — custom Yes/No dictionary | — |
| loyalty_no | nothing built-in — CTO Loyalty Number pattern (custom — below) | — |

**Selecting methods:** choose the built-in name dictionary, the eMail,
Phone and card patterns, and the US States dictionary together at Select
...
| loyalty / order references | CTO Loyalty Number pattern (custom — below) |
| store cities | CTO Service Cities dictionary (custom — below) |

### Custom method 1 — the CTO Service Cities dictionary

Built-in dictionaries know US states and country codes, but not which
Arizona communities CTO serves. A Service Cities dictionary lets
identification recognise CTO's store and customer cities — in the
`customers.city` column and in the correspondence alike. The asset is a
single-column CSV: `CTO-Service-Cities-Dictionary.csv` (header `term`, one
...
2. Enter a Name (`CTO Service Cities`), a short description, and a
   Category (add a new `CTO_Reference` category). Make sure the dictionary
   is enabled.
3. Choose **Upload Dictionary**, upload `CTO-Service-Cities-Dictionary.csv`,
   and set a Confidence Score of 0.7.
   `[SCREENSHOT: dictionary upload — CTO Service Cities]`
...
5. Click **Create Dictionary**. It now appears in the methods list — select
   ...
   `CTO-Product-Categories-Dictionary.csv` (`CTO Product Categories` — the
   six `category_cd` values) and `CTO-Tender-Types-Dictionary.csv`
   (`CTO Tender Types` — the four `pay_method_cd` values), then re-run
   identification on `products` and `payments` and watch the
   low-cardinality enum columns light up precisely — that's what
...
conditions, actions, and weightings — is method-authoring territory for the
Data Steward.

### Custom method 2 — an example CTO pattern

The correspondence (and the `loyalty_no` column the built-ins missed)
carries references like `CTO-200109`. A custom data pattern catches that
shape wherever it appears:

| Field | Value |
| --- | --- |
| Name | CTO Loyalty Number |
| Pattern | AAA-nnnnnn |
| RegEx | ^CTO-\d{6}$ |
| Matches | CTO loyalty numbers, in columns and inside documents |
| Suggested tag / term | Loyalty Number / Loyalty Number |

**You add this one:** the pattern above is a specification, not a finished
method. Create it via **Data Operations → Data Identification Methods →
Data Patterns → Add Pattern**, give the Column-Name Regex
`(?i)loyalty_?(no|num|number)` a 0.3 weight, the position signature
`AAA-nnnnnn` a 0.4 weight, and the content regex `^CTO-\d{6}$` a 0.3
weight (the three panes must sum to 1.0), with a confidence condition of
≥ 0.7 — then re-run identification on `customers` and watch the miss from
Part B disappear.

## Verify your work

- [ ] Data Identification has completed on `customers`, `payments`,
      `orders`, `employees` and `suppliers`.
- [ ] The matched columns carry Tags and linked Business Terms on the
      Details tab — card, email and contact PII where expected.
...
- [ ] The CTO Service Cities, Product Categories and Tender Types
      dictionaries are uploaded and appear in the methods list.
- [ ] You can explain the `card_no` triangulation — the HIGH identification
      tag, the failing PCI rule, and the NON-COMPLIANT attestation all
      point at one defect — and why `loyalty_no` needed a custom pattern.
...
| A column you expected wasn't matched | Its values may not appear in any dictionary and its shape may not match a pattern — `loyalty_no` is the deliberate example. A user-defined dictionary or pattern is the fix. |
...
| Cannot open Data Identification Methods | In PDC v11 the methods pages are gated to the **Data Steward** and **Data Storage Administrator** roles — work as `sofia.marin` or `leo.fischer`. |
...
correctly. Get identification right and a PCI assessment that spans both
the `payments` table and a folder of receipts becomes a filter rather than
a manual hunt.

Discuss: a PCI assessor asks for every place cardholder data is held. The
engine found `payments.card_no` — but the denied-return letter mentions a
card "ending 2006" and the POS export carries `card_last4` values. Which
of those are in scope, which method would flag each, and who decides?

## What's next

This completes the RETAIL workshop track. From here the natural
