# Workshop 1 — Connect the Data Sources (CPC)

*Cascade Precision Components scenario · PDC 11.0.0 · PDC at `https://pentaho.io`, sources in the Ubuntu VM at `192.168.1.200`*
...
Pentaho Data Catalog (PDC) first meets the Cascade Precision Components
(CPC) data estate. By the end, two very different worlds of data will be
visible in one place — and everything you do in the remaining workshops
depends on the connections you make here.

CPC is a fictional Pacific Northwest maker of precision hydraulic valves,
fittings and manifolds, with six plants from Portland to Spokane. Like
every manufacturer, it runs on two kinds of information. The first is
structured operations data — parts, bills of materials, suppliers, work
orders, lots, inspections, nonconformances, shipments — living in a
PostgreSQL database. The second, and in most companies far larger, is
unstructured content: certificates of conformance, mill certs, audit
reports, 8D corrective actions, recall letters, price lists. Industry
studies put unstructured content at 80–90% of what an organization
generates, and a certified manufacturer's quality records are the classic
case.

> **The business problem.** Before PDC, a CPC quality engineer asked
> *"which customers received parts from lot LOT-2026-0107?"* had to query
> the ERP, then dig through shipment paperwork, then check the certificate
> file share. The answer was scattered — and when that lot was recalled,
> every hour of searching was an hour of exposure. Connecting both the
> database and the document store to one catalog is the first concrete
> step toward answering the traceability question in one place, at recall
> speed.
...
- How to connect a PostgreSQL database and ingest its schema so the
  `cpc_mfg` tables become catalog assets.
...
- How to connect the MinIO object store holding CPC's documents, and
...
successful **Test Connection** proves the details work. This is
deliberate: a catalog that holds connection secrets for every system in
the business must protect them, and must never present a half-working
connection as trustworthy.
...
| --- | --- | --- |
| Example at CPC | PostgreSQL `cpc_mfg` | MinIO `cpc-documents` bucket |
...
of certificates and audit PDFs has no schema, so PDC scans the files and
reads their content with a different engine. Same catalog, two on-ramps.

## Before you begin

### Prerequisites

- The shared lab running with MFG loaded (`data_sources/lab`: `make up &&
  make load SCENARIO=MFG`), so PostgreSQL and MinIO are both up and
  verified.
- Workshop 0 users exist. Work as `petra.novak` (**Data Storage
  Administrator** — creating and ingesting data sources is her job;
  `nora.whitaker`'s Data Steward role also carries the rights).
...
- `data_sources/MFG/postgres-init/` — the manufacturing schema and sample
  data (loaded automatically by the lab loader).
- `data_sources/MFG/cpc-documents/` — the full document set (18 files
  across quality, compliance, ncr-reports, correspondence, erp-exports,
  and price-lists).
- `assets/cpc-datasources.csv` — both connections pre-filled for the
  Glossary Generator's **bulk loader**, the standard method in Part A.

Keep the two tables below handy — you will enter these values in the
steps. **Test Connection must succeed before either source comes online.**

### Database — Cascade_Manufacturing (PostgreSQL)

| Field | Value | Notes |
| --- | --- | --- |
| Data Source Name | Cascade_Manufacturing | Letters, digits, underscores only — no spaces |
...
| Database Name | cpc_mfg | The CPC manufacturing database |
| Schemas | cpc_mfg | Not `public` — an empty schema ingests "OK" and harvests nothing |
| User Name | pdc_user | Read-only — CONNECT, USAGE, SELECT |
| Password | catalog123! | Lab credential — change in production |

### Object store — Cascade_Documents (MinIO via the AWS S3 type)

| Field | Value | Notes |
| --- | --- | --- |
| Data Source Name | Cascade_Documents | Letters, digits, underscores only — no spaces |
| Data Source Type | AWS S3 | MinIO is S3-compatible; connect it with the AWS S3 type |
| Region | us-east-1 | Any value works for MinIO |
| Bucket Name | cpc-documents | The bucket holding CPC documents |
| Access Key | cpc_minio_user | MinIO access key — read-only |
...
The Glossary Generator's bulk loader registers every source in a CSV in
one pass — for each row it **creates** the data source over PDC's public
API, runs a **test-connection job** and waits for it, then triggers a
**metadata ingest** scoped to the new source (database rows only; object
stores have no schema to ingest). It is repeatable, secrets transit only
to PDC, and one CSV captures a whole scenario.
...
2. Enter the PDC connection: Base URL `https://pentaho.io` (the server
   root — the app adds `/api/public/...` itself), API version `v2`, and
   **Verify TLS off** for the lab's self-signed certificate.
3. Authenticate as `petra.novak` (Data Storage Administrator — creating
...
4. Choose the CSV: `assets/cpc-datasources.csv`. The panel previews both
   rows — the `postgres` row (`Cascade_Manufacturing`) and the `minio` row
   (`Cascade_Documents`). The values are already host-reachable
...
5. Tick **Dry run** and run once. The panel shows the exact (redacted)
   ...
   is `cpc_mfg`. Nothing has touched PDC yet.
...
7. One manual step remains: the **File System Scan is not on the public
   API**. In PDC, open `Cascade_Documents` and click **Scan Files** (a
   first full scan, defaults are fine). Monitor it on the Workers page.
   `[SCREENSHOT: Scan Files on Cascade_Documents]`
8. Verify in the Data Canvas: the eleven `cpc_mfg` tables and the six
   document folders are visible. If both are there, Parts B and C below
   are reference material — skip to Part D.
...
### Part B — Connect the manufacturing database manually (reference)

Use this path when you have no CSV, or to understand exactly what the
loader automates.

1. Sign in to `https://pentaho.io` as `petra.novak`. In the left
   navigation menu, click **Management**. The Manage Your Environment page
   opens.
2. In the Resources card, click **Add Data Source**. The Create Data
   Source page opens.
3. In **Data Source Name**, enter `Cascade_Manufacturing`. Names must
...
5. Enter the connection details from the table above: Host
   `192.168.1.200`, Port `5433`, Database Name `cpc_mfg`, User Name
   `pdc_user`, Password `catalog123!`.
   `[SCREENSHOT: Cascade_Manufacturing connection form]`
...
7. Click **Ingest Schema**. In the Select schemas dialog, choose the
   `cpc_mfg` schema — not `public`, which is empty. You can use
   include/exclude patterns to filter tables; for this lab, ingest
   everything in `cpc_mfg`.
   `[SCREENSHOT: Select schemas — cpc_mfg]`
8. Click **Ingest**, then **Create Data Source**. PDC loads the eleven
   tables and their columns as catalog assets: plants, employees,
   suppliers, parts, boms, purchase_orders, work_orders, lots,
   inspections, ncrs, shipments.

> **Why a read-only user?** You are connecting as `pdc_user`, which has
> only CONNECT, USAGE, and SELECT. The catalog only ever needs to read —
> it profiles and describes data, it never changes it. Using a
> least-privilege user means the catalog literally cannot modify CPC's lot
> records or dispositions, which is exactly the record-integrity control
> an AS9100 auditor expects. Good governance starts at the connection
> screen.
...
1. Back in **Management → Resources**, click **Add Data Source**. Name it
   `Cascade_Documents`.
2. In **Data Source Type**, select the **AWS S3** type. The form changes
   to ask for bucket details rather than a database name.
3. Enter: Endpoint `http://192.168.1.200:9000`, Access Key
   `cpc_minio_user`, Secret Key `minio_secret_123!`, Bucket
   `cpc-documents`. Use the VM IP for the Endpoint, not `localhost` — from
   ...
   `[SCREENSHOT: Cascade_Documents connection form]`
...
6. Monitor progress on the **Workers** page. When the scan completes, the
   six `cpc-documents` folders appear in the Data Canvas next to the
   database tables — compliance, correspondence, erp-exports, ncr-reports,
   price-lists, quality.
...
1. Run **Metadata Ingest** first — it reads the files' metadata into the
   catalog. Then select the `quality/` folder and choose **Process** to
   open the Choose Process pane and see the jobs PDC offers.
   `[SCREENSHOT: Choose Process pane on the quality folder]`
...
| PII Detection | An ML check that tags PII columns into an auto-created ML_PII glossary. Scope-limited (JDBC tables and CSV/TSV in specific languages) — and at CPC there is barely any PII for it to find |

For the `quality/` folder the realistic sequence is Metadata Ingest, then
Data Discovery, then Data Identification, optionally Calculate Trust
Score.
...
The two are mutually exclusive — Delete Empty Folders works on full
ingests only. For this lab, leave Incremental Ingest off (a full ingest)
so every document is read.
...
Optimizer, the tiering engine that can migrate, delete, and rehydrate
files to reduce storage cost.
...
> Source**. CPC would return here to model the savings from tiering old
> certificates into archive storage — mindful that AS9100 record-retention
> still applies wherever they land.

## Verify your work

You are done with Workshop 1 when all of the following are true:

- [ ] Cascade_Manufacturing is online and a Discovery search for `lots`
  returns the table with its columns.
- [ ] All eleven `cpc_mfg` tables are visible in the Data Canvas.
- [ ] Cascade_Documents is online and scanned, with the six document
...
| Test Connection fails for MinIO | Endpoint or keys wrong. Most often the Endpoint is `localhost` — from inside the PDC container that points at PDC itself; use `http://192.168.1.200:9000`. An IP endpoint also forces S3 path-style; a hostname makes the SDK prepend the bucket (`cpc-documents.host`) and fail |
| Ingest succeeds but no tables appear | The schema selection was `public`, which is empty. Re-ingest and select `cpc_mfg` |
| Scan finds no files | The bucket loaded empty. Re-run the lab loader (`make load SCENARIO=MFG`) — it verifies 18 objects |
| Source stays OFFLINE | Expected until Test Connection succeeds. Fix the details and test again |
| Bulk loader: Get token fails (401) | Wrong credentials, or the account lacks rights — use `petra.novak` or another Data Storage Administrator / admin. Confirm Base URL is the server root (`https://pentaho.io`) with Verify TLS off |
...
| Bulk loader: database ingest OK but 0 tables | `schemaNames` points at an empty schema. The CSV sets `cpc_mfg`; if edited, fix and re-run (the loader skips an existing source's create and re-runs the jobs) |
...
those behaviours essential for a certified manufacturer whose quality
records must be demonstrably controlled? Relate your answer to AS9100's
record-control requirements — and to the question the auditor will
actually ask: *"show me every system that holds traceability records."*
...
properties of the files. You will see that whether an asset is a lot table
or a certificate of conformance, PDC describes it through the same layers
of metadata.
