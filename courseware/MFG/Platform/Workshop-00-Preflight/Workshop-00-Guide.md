# Workshop 0 — Preflight: Provision Users & Roles (CPC)

*Cascade Precision Components scenario · PDC 11.0.0 · lab at `https://pentaho.io` (VM `192.168.1.200`)*
...
term, the Cascade Precision Components (CPC) team needs accounts — and each
...
CPC is an AS9100-certified manufacturer, and in a certified quality system
*who can change what* is itself an audited control. A business steward who
curates the glossary should not be able to rewrite a data source; an
...
workshop maps each CPC person to the role that matches their
responsibility.
...
> responsible for what — and a single mistake can change data an auditor
> expects to be controlled. When the AS9100 auditor asks *"who decided the
> lot table's CoC flag is a critical data element, and when?"*, the answer
> must be a named steward with the right role. Provisioning distinct,
> least-privilege accounts up front makes ownership visible and auditable —
> the same discipline the quality system already applies to dispositions
> and sign-offs.
...
- The seven default PDC roles across the Business and Expert tiers, and
  what each one can and cannot do.
- How to reset the catalog administrator's password in Keycloak and grant
  it the full set of roles.
- How to add each CPC user in Data Catalog and assign the least-privilege
...
PDC delegates authentication to Keycloak, the identity provider bundled
with the platform. Keycloak holds each user's identity and credentials in a
realm — for this instance the realm is `pdc` — and PDC authenticates users
against the `pdc-client` OIDC client. Authorization — what a signed-in user
may actually do — is expressed in PDC as roles and communities. In practice
this means two places do two jobs: you set or reset a password in
**Keycloak**, and you assign roles in **Data Catalog**.
...
view and explore; Expert Users create, ingest, and curate. When a user
holds more than one role the highest-level role determines effective
permissions, and the number of Expert seats you can assign is capped by
your licence — worth remembering before you hand out steward roles freely.
...
on a base role that narrow or widen access to specific assets (a Quality
community that sees only inspection and NCR glossaries, for example). This
workshop uses the default roles; communities become useful once CPC scopes
...
- The CPC roster below (`assets/users.csv`). All CPC emails follow the
  pattern `name@cascadeprecision.com`.
...
| Admin password | cascade | Lab credential — set non-temporary; change in production |

## The CPC roster — all seven PDC roles covered

The CPC team maps one persona to each role. Business Steward is held by the
four business-domain owners; there is deliberately only **one** Data
Steward.

| User | PDC role(s) | Why |
| --- | --- | --- |
| catalog.admin | ALL SEVEN ROLES | Course superuser / trainer. Password reset in Keycloak to `cascade` |
| nora.whitaker | Data Steward · Business Steward | The single Data Steward: connects sources, profiles, runs identification; stewards Production, Traceability, Plant Operations |
| felix.okonkwo | Business Steward | Manufacturing engineer; stewards Parts & BOM and Suppliers & Purchasing (Part Number, Unit Cost, ASL Status) |
| yuki.mori | Business Steward | Quality lead; stewards Quality & Inspection and Nonconformance (all CDE) — owns the CoC and MRB stories |
| silas.grant | Business Steward | Stewards Shipments & Customers and Records & Documents — owns the recall story |
| petra.novak | Data Storage Administrator | Storage custodian: creates/ingests data sources, monitors utilisation; fills the app's custodian slot |
| andre.gibson | Data Developer | Authors the Workshop 4 business rules and domain logic (this role owns Business Rules — Admin cannot create them) |
| mia.torres | Data User | Business Analyst persona — reads, searches, views data assets |
| owen.fitch | Business User | Lightest tier: views the glossary and policies only — the contrast case to Mia |
...
> `data_sources/MFG/domain_pack/manufacturing.people.json`); the Glossary
...
1. Open the Keycloak admin console at `https://pentaho.io/keycloak` and
   sign in with the Keycloak (master-realm) administrator credentials.
...
3. Go to **Users**, search for `catalog.admin`, and open the account. If
   it does not exist yet, click **Add user**, set the username to
   `catalog.admin` and email `catalog.admin@cascadeprecision.com`, and
   save.
4. Open the **Credentials** tab, click **Reset password**, enter `cascade`
   in both fields, and — importantly — turn **Temporary off** so the
   password works immediately without a forced reset. Save.
...
### Part B — Add the CPC users (Data Catalog)

Sign in to PDC as `catalog.admin` / `cascade`. Confirm the Administration
...
3. Enter the user's details — username and email from the roster (for
   example `yuki.mori` / `yuki.mori@cascadeprecision.com`).
4. Click **Add Roles**, tick the role or roles listed for that user in the
   ...
   `[SCREENSHOT: Create User — yuki.mori with Business Steward]`
5. Click **Done** to create the user. Repeat for all eight CPC users. If
...
2. On the **Credentials** tab, **Reset password** to the shared lab
   password `cascade`, with Temporary turned **off**. Save.
...
> analyst any steward role. `yuki.mori` can shape the glossary but cannot
> touch a data source; `mia.torres` can read the catalog but cannot
> administer users; `andre.gibson` can author business rules that even the
> Admin role cannot. That separation is exactly what a quality auditor
> expects to see — the data-governance twin of the MRB sign-off rule.
...
| W1 create + ingest both sources, Scan Files | `petra.novak` | Data Storage Administrator creates data sources (Nora's Data Steward role also can) |
| W2 explore structure & metadata | `mia.torres` | Data User views data assets (repeat one step as `owen.fitch` to see the Business User boundary) |
| W3 import + review the glossary | `yuki.mori` (any Business Steward) | Business Steward creates/imports glossaries |
| W3 link terms to columns | `nora.whitaker` | Needs data-source write — her Data Steward side |
| W4 profile the tables | `nora.whitaker` | Data Steward runs profiling jobs |
| W4 author the business rules | `andre.gibson` | Data Developer owns Business Rules in v11 |
| W5 dictionaries, patterns, identification | `nora.whitaker` | Data Steward owns Data Identification Methods (so does Petra) |

## Verify your work

You are done with Workshop 0 when all of the following are true:

- [ ] `catalog.admin` signs in to PDC with the password `cascade` and sees
  the Administration area.
- [ ] All eight CPC users exist under Users & Communities, each with the
  role(s) from the roster — covering all seven PDC roles.
- [ ] A Business Steward (for example `yuki.mori`) can open Business
  Glossary but not create a data source.
- [ ] A Data Storage Administrator (`petra.novak`) can add a data source,
...
Access control is the quiet foundation of every catalog. A certified
manufacturer already lives this discipline on the shop floor — only the MRB
can approve a use-as-is, only quality can release a lot. By separating
identity in Keycloak from authorization in PDC, and by assigning each CPC
user only the role their work requires, you extend the same discipline to
the data about the shop floor. As the cohort moves into Workshop 1 and
beyond, keep asking the governance question this workshop sets up — not
just *"can I do this?"* but *"should this role be able to?"*

## What's next

With the cast in place, Workshop 1 connects the CPC data estate — the
`cpc_mfg` manufacturing database and the `cpc-documents` object store —
performed by `petra.novak`, whose Data Storage Administrator role exists
