...
-- Grant least-privilege read access to the lhp_clinical schema.
GRANT CONNECT ON DATABASE lhp_clinical TO pdc_user;
GRANT USAGE ON SCHEMA lhp_clinical TO pdc_user;
GRANT SELECT ON ALL TABLES IN SCHEMA lhp_clinical TO pdc_user;

-- Ensure any tables created later are also readable by pdc_user.
ALTER DEFAULT PRIVILEGES IN SCHEMA lhp_clinical
