...
-- Grant least-privilege read access to the cpc_mfg schema.
GRANT CONNECT ON DATABASE cpc_mfg TO pdc_user;
GRANT USAGE ON SCHEMA cpc_mfg TO pdc_user;
GRANT SELECT ON ALL TABLES IN SCHEMA cpc_mfg TO pdc_user;

-- Ensure any tables created later are also readable by pdc_user.
ALTER DEFAULT PRIVILEGES IN SCHEMA cpc_mfg
