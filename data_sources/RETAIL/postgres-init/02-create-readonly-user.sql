...
-- Grant least-privilege read access to the cto_retail schema.
GRANT CONNECT ON DATABASE cto_retail TO pdc_user;
GRANT USAGE ON SCHEMA cto_retail TO pdc_user;
GRANT SELECT ON ALL TABLES IN SCHEMA cto_retail TO pdc_user;

-- Ensure any tables created later are also readable by pdc_user.
ALTER DEFAULT PRIVILEGES IN SCHEMA cto_retail
